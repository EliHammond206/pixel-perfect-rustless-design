              ...
              {registrationSteps.map((step, index) => (
                <CarouselItem key={index} className="pl-2 md:pl-4 basis-full sm:basis-1/2 lg:basis-1/3">
                  <div className="flex flex-col items-center text-center space-y-4">
                    {/* Show company name (if companies prop is provided), else show step number */}
                    {companies && companies[index] ? (
                      <div className="bg-gradient-to-r from-[#06beb6] to-[#48b1f3] text-white rounded-full px-4 py-2 min-w-[70px] flex items-center justify-center text-base font-bold mb-2 uppercase shadow-sm tracking-wide">
                        {companies[index]}
                      </div>
                    ) : (
